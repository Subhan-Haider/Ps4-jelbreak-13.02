...
| :------------------------ | :---------- | :--------------- |
| 5.05–13.50                | 1.01–12.02  | 1.01-13.00       |
...
| 7.00-13.50 (PS4) / 1.00-12.00 (PS5) |
| :--------- |

* By default Lapse is used from 7.00 to 12.02, and Poopsploit from 12.50-13.00. 
* **Firmwares 13.02 - 13.50 are USERLAND ONLY.** Kernel access (GoldHEN) is not yet available for these versions.
* PS5 Experimental support added for userland and initial kernel exploration.

# FAQ & Troubleshooting

**Q: Will this work on 13.02 or above?**
Only the Userland exploit functions on 13.02+. This allows you to run basic JS payloads and explore the system, but you cannot achieve a full jailbreak (GoldHEN) yet because Sony patched the Poopsploit/Netctrl vulnerability.
...
If you have modified the source files or want to build the latest version for Windows:
1. Ensure you have **Node.js** installed.
2. Run `npm install` in the root directory.
3. Execute `.\build_win.bat`.
4. The compiled files will be located in the `dist/` directory. Use these files for hosting.
...
# Themes 
Themes can be added by simply copying the folder to `/download0/themes/`. The default theme provides a good example of how a theme should be written.

Currently included themes:
- **Default**: The standard VAF look.
- **Midnight**: A premium dark theme with deep blue accents and a custom background.

# Payloads
Vue After Free comes preloaded with several payloads:
- **FTP**: `ftp-server.ts` - Sandbox FTP server.
- **Kernel Dumper**: `kernel-dumper.ts` - Simulated kernel dumping utility.
- **System Info**: `sys-info.ts` - Display console firmware and hardware model.
- **Reboot**: `reboot.ts` - Soft reboot the console.
- **WebUI**: `web-ui.ts` - Example for browser-based userland code.
- **ELFLDR**: `elfldr.elf` - Load post-exploit payloads.
